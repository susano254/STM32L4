...
void RCC_SET_PLL_SRC(uint32_t pllSrc){
	...
	if(pllSrc == RCC_PLLCFGR_PLLSRC_MSI) RCC->PLLCFGR |= RCC_PLLCFGR_PLLSRC_MSI;
	else if(pllSrc == RCC_PLLCFGR_PLLSRC_HSI) RCC->PLLCFGR |= RCC_PLLCFGR_PLLSRC_HSI;
	else if(pllSrc == RCC_PLLCFGR_PLLSRC_HSE) RCC->PLLCFGR |= RCC_PLLCFGR_PLLSRC_HSE;
	else RCC->PLLCFGR |= 0x00 << RCC_PLLCFGR_PLLSRC_Pos;
}
...
uint8_t RCC_SET_SYSCLK(uint32_t sysClock){
	if(sysClock > RCC_CFGR_SW_PLL | sysClock < RCC_CFGR_SW_MSI) return 0;
	RCC->CFGR &= ~RCC_CFGR_SW_Msk;
	RCC->CFGR |=  sysClock;
	return 1;
}

uint8_t RCC_SET_AHB_PRESCALER(uint32_t prescaler){
	RCC->CFGR &= ~RCC_CFGR_HPRE_Msk;
	RCC->CFGR |= prescaler;
}

uint8_t RCC_SET_APB1_PRESCALER(uint32_t prescaler){
	RCC->CFGR &= ~RCC_CFGR_PPRE1_Msk;
	RCC->CFGR |= prescaler;
}

uint8_t RCC_SET_APB2_PRESCALER(uint32_t prescaler){
